...
class Triggers(loader.Module):
    ...
    strings_ru = {
        ...
        "not_valid": "<emoji document_id=5210952531676504517>❌</emoji> Триггер не валиден!",
        "error": "<emoji document_id=5210952531676504517>❌</emoji> Неожиданная ошибка. Обратитесь к разработчику модуля или попробуйте изменить данные: {e}",
    ...
    @loader.command(ru_doc="Включить или выключить статус модуля")
    async def tstatus(self, message: Message):
        """Toggle module status"""
        self.status = not self.status
        status_text = "enabled" if self.status else "disabled"
        await utils.answer(message, f"Module status is now {status_text}")

    @loader.command(ru_doc="Разрешить или запретить выполнение любых команд юзербота")
    async def tallowinvoke(self, message: Message):
        """Toggle allow_invoke"""
        self.allow_invoke = not self.allow_invoke
        allow_invoke_text = "enabled" if self.allow_invoke else "disabled"
        await utils.answer(message, f"Allow invoke is now {allow_invoke_text}")

    @loader.command(ru_doc="Установить время троттлинга")
    async def tthrottletime(self, message: Message):
        """Set throttle time"""
        args = utils.get_args_raw(message).split()
        ...
        self.cache.ttl = float(args[0])
        await utils.answer(message, f"Throttle time is now set to {args[0]} seconds")

    @loader.command(ru_doc="Показать текущие настройки")
    async def tsettings(self, message: Message):
        """Show current settings"""
        settings = f"Status: {'enabled' if self.status else 'disabled'}\n"
        settings += f"Allow invoke: {'enabled' if self.allow_invoke else 'disabled'}\n"
        settings += f"Throttle time: {self.cache.ttl} seconds"
        await utils.answer(message, settings)
