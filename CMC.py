...
class CMCMod(loader.Module):
    ...
    async def get_message_stats(self, chat_id, user_id, is_private=False):
        ...
        while True:
            if is_private:
                history = await self._client(telethon.functions.messages.GetHistoryRequest(
                    peer=chat_id,
                    offset_id=offset_id,
                    offset_date=None,
                    add_offset=0,
                ...
                messages = await self._client(telethon.functions.messages.SearchRequest(
                    peer=chat_id,
                    q="",
                    filter=telethon.types.InputMessagesFilterEmpty(),
                    min_date=None,
                    max_date=None,
                    offset_id=offset_id,
                    add_offset=0,
    ...
    async def get_chat_total_stats(self, chat_id):
        ...
        media_messages = await self._client(telethon.functions.messages.SearchRequest(
            peer=chat_id,
            q="",
            filter=telethon.types.InputMessagesFilterPhotoVideo(),
            min_date=None,
            max_date=None,
            offset_id=0,
            add_offset=0,
